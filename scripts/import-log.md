...
## CRM capture identity + legacy reverse retirement — 2026-09-14 (prepared, not imported)

- **Files**: `n8n-workflows/capture-email-contacts.json`, `n8n-workflows/clickup-contact-review-supabase-reverse.json`; read-only lookup, synthetic tests/fixture and `scripts/docs/crm-capture-identity-primer.md` complete the separate seven-file build.
- **Behavior**: preserve known/rejected/merged identities; hold ambiguity; one create attempt with explicit HTTP/transport handling and fresh lookup; defer named exceptions until successful links and attachment work finish. The old reverse source is inactive with all nine nodes retained. Weekly reconciler remains the review writer.
- **Validation**: actual JavaScript/expression tests plus pinned offline PostgreSQL role/RLS, identity-path, whitespace, scalar owner completeness and concurrent INSERT checks. Both workflow validators report zero HIGH findings. Four capture P011 advisories remain: two existing attachment references are untouched; Resolve uses the complete Participants collection keyed by email, and Build Junction Rows uses the one Create Interaction ID. They do not depend on transformed item pairing.
- **New pattern**: PostgreSQL `E'\v'` means letter v, not vertical tab. The lookup uses the exact JavaScript whitespace character set; synthetic fixtures include leading/trailing v, actual VT, NBSP and BOM. Conditional Merge waiting was removed: the retained Merge Participants name is a Code gate on an explicitly ordered v1 branch.
- **Limits**: no full local n8n engine test, live import, deactivation, new SQL/guard deployment, live synthetic write or historical apply. Supervised engine/error-visibility smoke and human merge/deployment gates remain; see the primer.
- **Receipts**: exact candidate/test/review/merge/deployment hashes and results belong in `~/alex-os-outbox/build-runs/357-2026-09-06-01a074b6/crm-recovery-coordination-2026-09-13/n8n-approved-20260914/`. A later review verdict is not asserted here and does not require editing the reviewed candidate to record it.

- **Consolidated review repair**: locally hold invalid/oversized/control/Unicode and >1,000 overflow participants; exclude them from both RPC batches while preserving valid links and indexed exceptions. The read-only lookup now rejects non-BYPASSRLS/non-superuser callers before silent RLS filtering; SQL privileges remain invoker-bound. Capture ships active:false, and edited/new node positions are distinct. Added mixed-input/boundary/overflow and unsupported junction-receipt tests. The primer now gates activation on actual credential visibility, active-state and junction shape/readback, distinguishes the unmodified-base preflight from the candidate, and states preserved BCC exclusion plus the fatal-attachment diagnostic limit. Guard sentinel and committed operational constants remain unchanged; parent-only review abstractions are never import artifacts. A second independent verdict is still recorded outside source.
